...
class PingUDP extends _ping.default {
  async send() {
    ...
    const socket = new _dgram.default.createSocket(new _ip.default(this.result.ip).is4 ? 'udp4' : 'udp6');
    const buffer = this.options.buffer ? this.options.buffer : this.options.body ? Buffer.from(this.options.body) : Buffer.alloc(this.options.bytes);
    let timeout;
    ...
    if (this.options.broadcast) {
      socket.send(buffer, 0, buffer.length, this.result.port, this.result.ip, concb);
    } else {
  ...
  static sendAsync(options = {}, callback = () => {}) {
    return new Promise((resolve, reject) => {
      new PingUDP(options).on('result', result => {
        resolve(result);
  ...
  async scan() {
    this.options.ports = !this.options.ports ? [67, 68, 69, 123, 161, 162, 445, 514, 19132] : this.options.ports;

...
var _rawSocket = _interopRequireDefault(require("raw-socket"));

var _ip = _interopRequireDefault(require("@wnynya/ip"));

var _ping = _interopRequireDefault(require("./ping.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class PingICMP extends _ping.default {
  async send() {
    ...
    this.addressFamily = new _ip.default(this.result.ip).is4 ? _rawSocket.default.AddressFamily.IPv4 : _rawSocket.default.AddressFamily.IPv6;
    this.protocol = new _ip.default(this.result.ip).is4 ? _rawSocket.default.Protocol.ICMP : _rawSocket.default.Protocol.ICMPv6;
    const timestamp = new Date().getTime();

    const socket = _rawSocket.default.createSocket({
    ...
    socket.send(buffer, 0, buffer.length, this.result.ip, () => {
      socket.setOption(new _ip.default(this.result.ip).is4 ? _rawSocket.default.SocketLevel.IPPROTO_IP : _rawSocket.default.SocketLevel.IPPROTO_IPV6, new _ip.default(this.result.ip).is4 ? _rawSocket.default.SocketOption.IP_TTL : _rawSocket.default.SocketOption.IPV6_TTL, this.options.ttl);
    }, (error, bytes) => {
  ...
  #parseBuffer(buf, src) {
    ...
    if (new _ip.default(this.result.ip).is4) {
      offset = (buf[0] & 0x0f) * 4;
    }
  ...
  #parseTypeCode(type, code) {
    const types = ['ECHO_REPLY', 'UNASSIGNED_1', 'UNASSIGNED_2', 'DESTINATION_UNREACHABLE', 'SOURCE_QUENCH', 'REDIRECT', 'ALTERNATE_HOST_ADDRESS', 'UNASSIGNED_7', 'ECHO', 'ROUTER_ADVERTISEMENT', 'ROUTER_SECTION', 'TIME_EXCEEDED', 'PARAMETER_PROBLEM', 'TIMESTAMP', 'TIMESTAMP_REPLY', 'INFORMATION_REQUEST', 'INFORMATION_REPLY'];
    const type_3_codes = ['NET_UNREACHABLE', 'HOST_UNREACHABLE', 'PROTOCOL_UNREACHABLE', 'PORT_UNREACHABLE', 'FRAGMENTATION_NEEDED', 'SOURCE_ROUTE_FAILED', 'NET_UNKNOWN', 'HOST_UNKNOWN', 'HOST_ISOLATED', 'NET_PROHIBITED', 'HOST_PROHIBITED', 'NET_SERVICE_UNREACHABLE', 'HOST_SERVICE_UNREACHABLE', 'COMM_PROHIBITED', 'HOST_PRECEDENCE_VIOLATION', 'PRECEDENCE_CUTOFF'];
    const type_5_codes = ['NETWORK', 'HOST', 'SERVICE_AND_NETWORK', 'HOST_AND_NETWORK'];
  ...
  #createBuffer(data) {
    const type = Buffer.from([new _ip.default(this.result.ip).is4 ? 0x08 : 0x80, 0x00]);
    ...
    return _rawSocket.default.writeChecksum(buffer, 2, _rawSocket.default.createChecksum(buffer));
  }
  ...
  static sendAsync(options = {}, callback = () => {}) {
    return new Promise((resolve, reject) => {
      new PingICMP(options).on('result', result => {
        resolve(result);
  ...
  async traceroute() {
    ...
    this.result.hops = [];
    this.options.ttln = Math.max(1, Math.min(256, this.options.ttln ? this.options.ttln * 1 : 1));
    this.options.ttlx = Math.max(1, Math.min(256, this.options.ttlx ? this.options.ttlx * 1 : 64));
    this.options.timeoutx = Math.max(1, Math.min(256, this.options.timeoutx ? this.options.timeoutx * 1 : 8));
